...
const TemplateRenderer: React.FC<ITemplateRendererProps> = (props) => {
                ...
                filterNameElements.forEach((element) => {
                    if (element) {
                        try {
                            ...
                            iconContainer.style.position = 'relative'; // For tooltip positioning relative to the icon

                            // Append the icon container to the target element
                            element.appendChild(iconContainer);

                            // Render the FontIcon and Tooltip inside the container using React's useEffect
                            ReactDOM.render(
                                <div style={{ position: 'relative', display: 'inline-block' }}>
                                    <FontIcon
                                        onClick={(e) => alert('hi there')}
                                        iconName="Info"
                                        aria-label="Info"
                                        style={{
                                            fontSize: 16,
    ...
    const handleMouseEnter = (event: React.MouseEvent<HTMLDivElement, MouseEvent>, templateAsHtml: Document) => {
        const filterNameElements = templateAsHtml.querySelectorAll('.filter--name');
        filterNameElements.forEach((element) => {
            if (element) {
                (element as HTMLElement).style.display = 'flex';
                (element as HTMLElement).style.gap = '3px';
                (element as HTMLElement).style.alignContent = 'center';
                (element as HTMLElement).style.alignItems = 'flex-start';
            }
        });
        alert('hi there');
    };

    const TooltipComponent = (templateAsHtml: Document) => (
        <div
            onMouseEnter={(event) => handleMouseEnter(event, templateAsHtml)}
